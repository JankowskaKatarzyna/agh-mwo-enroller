...
public class MeetingRestController
{
    ...
    public ResponseEntity<?> searchByTitle(@PathVariable("title") String title)
    {
        Collection<Meeting> meetings = meetingService.getAll();
        List<Meeting> found =
                        meetings.stream().filter(meeting -> meeting.getTitle().contains(title))
    ...
    public ResponseEntity<?> searchByParticipantName(
                    @PathVariable("participantLogin") String participantLogin)
    {
        ...
        List<Meeting> found = meetings.stream()
        ...
        if (meetingService.alreadyExist(meeting))
        {
            return new ResponseEntity<String>(
                            "Unable to add. A meeting with title " + meeting.getTitle()
                                            + " and date " + meeting.getDate() + " already exist.",
                            HttpStatus.CONFLICT);
        }
                    ...
                    @RequestBody Map<String, String> json)
    {
        ...
        {
            return new ResponseEntity<String>(
                            "Unable to find participant login in the request body",
        ...
        return new ResponseEntity<Collection<Participant>>(currentMeeting.getParticipants(),
                        HttpStatus.OK);
    }

    @RequestMapping(value = "{id}/participants/{login}", method = RequestMethod.DELETE)
    public ResponseEntity<?> removeParticipant(@PathVariable("id") long id,
